...
filenames = os.listdir('/Users/kewang/CS294HW/homework/hw1/Results_Behavior_Cloning_Hopper')
...
for filename in filenames:
    if filename[0:5] == 'testo':
        print(filename)
        num_echos = int(filename[16:-11])
        print(num_echos)
        list_raw = list(pd.read_csv('/Users/kewang/CS294HW/homework/hw1/Results_Behavior_Cloning_Hopper/'+filename))[0:-2]
        data_raw = []
...
plt.xlabel("Log 10 (Number of Epochs)",fontsize = 18,labelpad = 0)
plt.ylabel("Policy's Reward",fontsize = 18,labelpad = 0)
plt.title("BC Performance over Epochs\n Number (Hopper Task)",fontsize = 18)
