...
#                           ----------PROBLEM 2----------
#========================================================================================#  
def build_mlp(input_placeholder, output_size, scope, n_layers=2, size=64, activation=tf.tanh, output_activation=None):
    """
    ...
    with tf.variable_scope(scope):
        ...
        #     inph = tf.nn.relu(tf.layers.dense(inph,size))
        # output_placeholder = tf.layers.dense(inph,output_size)
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_logits_na = build_mlp(input_placeholder=sy_ob_no, output_size=self.ac_dim, scope="policy_kw", n_layers=self.n_layers, size=self.size,activation=tf.nn.relu)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            sy_mean = build_mlp(input_placeholder=sy_ob_no, output_size=self.ac_dim, scope="policy_kw", n_layers=self.n_layers, size=self.size,activation=tf.nn.relu)
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            distribution_m = tf.contrib.distributions.MultivariateNormalDiag(loc=sy_mean, 
            scale_diag=tf.exp(sy_logstd)) 
            sy_logprob_n = distribution_m.log_prob(sy_ac_na)
            # sy_logprob_n = None
        return sy_logprob_n
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for path_re in re_n:
                results = []
                rk = 0
                for reward in reversed(path_re):
                    rk = rk*gamma+reward
                    results.append(rk)
                results = results[::-1]
                q_n.extend(results)
            ...
            for path_re in re_n:
                results = []
                su = 0
                for ii in range(len(path_re)):
                    su += np.power(gamma,ii)*path_re[ii]
                for ii in range(len(path_re)):
                    results.append(su)
                q_n.extend(results)
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            #raise NotImplementedError
            adv_n = adv_n = (adv_n - np.mean(adv_n)) / np.std(adv_n) # YOUR_CODE_HERE
        return q_n, adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        _, loss_value = self.sess.run([self.update_op, self.loss], feed_dict={self.sy_ob_no: ob_no,self.sy_ac_na: ac_na,self.sy_adv_n: adv_n})
        # YOUR_CODE_HERE
        # raise NotImplementedError
