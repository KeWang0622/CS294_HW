...
class ModelBasedRL(object):
    ...
    def _train_policy(self, dataset):
        ...
        losses = []
        for jj in range(self._training_epochs):
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            pred_states = []
            print(actions.shape)
            for itr in range(actions.shape[0]):
                if itr == 0:
                    ss = states[0]
                ss = self._policy.predict(ss, actions[itr])
                pred_states.append(ss)
                # pred_states.append(self._policy.predict(states[0], actions[itr])) 
            ### PROBLEM 1
            ### YOUR CODE HERE
            #raise NotImplementedError
    ...
    def run_q3(self):
        ...
        for itr in range(self._num_onpolicy_iters + 1):
            ...
            logger.info('Training policy...')
            #raise NotImplementedError
            rollouts = self._gather_rollouts(self._policy,self._num_onpolicy_rollouts)
            ### PROBLEM 3
            ### YOUR CODE HERE
            ...
            ### PROBLEM 3
            ### YOUR CODE HERE
            dataset.append(rollouts)
            logger.info('Appending dataset...')
            #raise NotImplementedError
            new_dataset = rollouts
