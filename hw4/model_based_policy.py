...
class ModelBasedPolicy(object):
    ...
    def _setup_action_selection(self, state_ph):
        ...
        for tt in range(10):
            sample_uniform = tf.random_normal([self._num_random_action_selection,self._horizon,self._action_dim],mean = mean_a,stddev = std_a)
            mask_greater_m = tf.math.greater(sample_uniform,-1)
            mask_less_m = tf.math.less_equal(sample_uniform,-1)
            ones = tf.ones([self._num_random_action_selection,self._horizon,self._action_dim])
            sample_u1 = tf.multiply(sample_uniform,tf.cast(mask_greater_m,dtype = tf.float32)) - tf.multiply(tf.cast(ones,dtype = tf.float32),tf.cast(mask_less_m,dtype = tf.float32))
            mask_greater_p = tf.math.greater(sample_u1,1)
            mask_less_p = tf.math.less_equal(sample_u1,1)
            sample_uniform = tf.multiply(sample_u1,tf.cast(mask_less_p,dtype = tf.float32)) + tf.multiply(tf.cast(ones,dtype = tf.float32),tf.cast(mask_greater_p,dtype = tf.float32))
            print(tf.shape(sample_uniform))
            # sample_uniform[tf.where(sample_uniform<-1)]=-1
            #ost_all = 0 
            for num_horizon in range(self._horizon):
                ...
                #     ttmin = tf.reduce_min(cost_t)
                #     j_min = tf.argmin(cost_t)
            j_min = tf.nn.top_k(-cost_all,200).indices
            print(j_min)

            #print(j_min)
            ac = tf.zeros([200,15,6])
            best_actions = []
            for ii in range(200):
                if ii == 0:
                    best_actions.append(tf.reshape(sample_uniform[j_min[0],:,:],[1,15,6]))
                else:
                    best_actions.append(tf.reshape(sample_uniform[j_min[ii],:,:],[1,15,6]))
                    # best_actions = tf.reshape(best_actions,[ii+1,15,6])
            best_actions = tf.stack(best_actions)
            mean_a,std_a = tf.nn.moments(best_actions,axes = [0,1,2])

        jj_min = tf.argmin(cost_all)
        best_action = sample_uniform[jj_min,0,:]
