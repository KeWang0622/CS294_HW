...
class QLearner(object):
  ...
  def __init__(
    ...
    # tar_val = self.rew_t_ph + (1-self.done_mask_ph)*gamma*tf.reduce_max(q_value_tar,axis=1)
    ...
    if double_q == 1:
      ...
      # self.q_value_next = q_func(obs_tp1_float,self.num_actions,scope = 'q_func_scope', reuse = False)
      self.q_value = q_func(obs_t_float,self.num_actions,scope = 'q_func_scope', reuse = False)
      ...
      q_val = tf.reduce_sum(self.q_value * tf.one_hot(self.act_t_ph, self.num_actions), axis=1)
      self.total_error = huber_loss(q_val - tf.stop_gradient(tar_val), delta=1.0)     
    #self.total_error = 0.5 * tf.reduce_sum(tf.square(q_val - tf.stop_gradient(tar_val)))
    # YOUR CODE HERE
  ...
  def step_env(self):
    ...
    if not self.model_initialized:
      ...
      obs_q = self.replay_buffer.encode_recent_observation()
      action_v = self.session.run(self.prc,feed_dict={self.obs_t_ph: [obs_q]})[0]
      action = action_v.copy()
  ...
  def log_progress(self):
    ...
    if self.t % self.save_every_n_steps == 0 and self.model_initialized:
      q_log = ({'t_log':self.t_log,'mean_episode_reward_log':self.mean_episode_reward_log,
                'best_mean_episode_reward_log':self.best_mean_episode_reward_log,'episodes_log':self.episodes_log,
                'exploration_log':self.exploration_log,'learning_rate_log':self.learning_rate_log})
      filename = 'results/Pong'+'KWANG'+str(self.t)+str(time.time())+'_data.pkl'
